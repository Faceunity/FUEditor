...
vec3 sampleEnv(vec3 R0){
	...
	return texture2D(tex_light_probe,vec2(phi,theta)).xyz*4.0;
}
...
vec4 shader_main(){
	...
	if (selfDot(dPds_frag) > 0.0 && selfDot(dPdt_frag) > 0.0){
	...
	return vec4(C_brdf+(C_refl-C_brdf)*(fresnel(dotNV)*Kr*smoothness),C_tex.w);
}
