(function(){
	...
	var bg_boards = boards.filter(function(board){
		if(board.name.search("_fcbg")!=-1){
			board.last = Date.now();
			board.isActive = true;
			return true;
		}
	...
	var boards = boards.filter(function(board){
		return board.name.search("_fcbg")==-1 && board.name.search("_ar")==-1;
	});
	
	ar_meshs.forEach(function(armesh){
		arTex.push(FaceUnity.LoadTexture(armesh.path));
	});
...
　　}
		...
		try{
			...
			if(this.name.search("_bg")!=-1){
				for (var j=0;j<this.texture_frames_bk.length;j++){
					...
					this.matp = g_params["matp"];
				}
			}
			if(this.name.search("_fcp")!=-1){
				if(g_params.isLandscape>0.5)this.isActive = false;
				for (var j=0;j<this.texture_frames_bk.length;j++){
					this.texture_frames[j].v=[-w/2,h/2,params.focal_length,
	...
	Mesh.prototype.orientationChange = function(){
		...
		if(this.name.search("_fc")!=-1){
			this.isActive = bisLandscape == (this.name.search("_fcl")!=-1);
		}
	}
	...
	Mesh.prototype.triggerThis=function(now){
		...
		this.last = now;
		this.isActive = true;
		//console.log("thriggerThis",this.name);
	}
	...
	for(var i = 0;i<boards.length;i++){
		...
		if(nmesh.name.search("_fcp")!=-1 || nmesh.name.search("_fcl")!=-1 || nmesh.name.search("_bg")!=-1||nmesh.name.search("_fcbg")!=-1){
	...
	var reextract_mesh_ref_lst = meshlst.filter(function pred(mesh){
		return mesh.name.search("_fcp")!=-1 || mesh.name.search("_fcl")!=-1 || mesh.name.search("_bg")!=-1;
	});
	...
	var fc_mesh_ref_lst = reextract_mesh_ref_lst.filter(function pred(mesh){
		return mesh.name.search("_fcp")!=-1 || mesh.name.search("_fcl")!=-1;
	});
	...
	for(var i = 0;i<meshlst.length;i++){
		...
		for(var j = 0;j<nmesh.triggerNextNodes.length;j++){
			nmesh.triggerNextNodesRef.push(meshlst[nmesh.triggerNextNodes[j]]);
		}
	}
	////////////////////////////////////
	return {
		...
		Render:function(params){
			try{
				...
				for(var i = 0; i < meshlst.length; i++)meshlst[i].renderEvent(params,now,mat_cam,false);
		...
		RenderNonFace:function(params){
			try{
				...
				for(var i = 0; i < meshlst.length; i++)meshlst[i].renderEvent(params,now,undefined,true);
