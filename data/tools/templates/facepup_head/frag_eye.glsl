...
vec3 StandardSurfaceShading(const FragmentState state, const Light light, const float visibility){
	vec3  h = normalize(state.vertex_eye + light.l);
    ...
    float diffuse = M_INV_PI;
    // Energy conservative wrap diffuse to simulate subsurface scattering
    diffuse *= Fd_Wrap(dot(state.normal, light.l), diffuse_wrap);
    ...
    // it into account in the diffuse lobe
    vec3 color = Fd;
    return (color * light.colorIntensity.rgb) * (light.attenuation * visibility);
}
...
vec3 SpecularIrradiance(vec3 r, float Roughness)
{
	...
	const float ReflectionCubemapMaxMip = 7.0;
...
{
	return N.xyz*2.0 - vec3(1.0,1.0,1.0);
}
...
void SetupMaterial(inout FragmentState s)
{
	vec2 sclera_uv = s.main_uv * sclera_size - (sclera_size-1.0)*0.5;
	vec4 sclera_color = SAMPLER_2D(tex_sclera, sclera_uv);
    float mask = sclera_color.a;

   
		vec4 C_mask = SAMPLER_2D(tex_changemask,st_frag);
		//if(C_mask.r > 0.5) {
			vec3 C_hsv = rgb2hsv(sclera_color.rgb);
			C_hsv.r = color_change2.r;
			C_hsv.g *= color_change2.g;
			C_hsv.b *= color_change2.b;
			if(C_hsv.b>255.0) C_hsv.b = 255.0;
			sclera_color.rgb = hsv2rgb(C_hsv);
		//}		
	

    vec3 iris_color_shifted = rgb2hsv(sclera_color.rgb);
    iris_color_shifted.r = saturate(iris_color_shifted.r +iris_color_hue_shift.r); 
    iris_color_shifted.g = saturate(iris_color_shifted.g +iris_color_hue_shift.g); 
    iris_color_shifted = hsv2rgb(iris_color_shifted);

    sclera_color.rgb = mix(sclera_color.rgb*sclera_intensity, iris_color_shifted*iris_intensity, mask);
	sclera_color.rgb = sRGBToLinearFast(sclera_color.rgb) * mix(sclera_intensity, iris_intensity, mask);
	s.albedo = sclera_color.rgb;
	s.opacity = 1.0;
    ...
    m = metallic_factor;
	g = gloss_factor;
	s.ao = occlusion_intensity;

	float spec = 0.04;
	s.reflectivity = mix(vec3(spec,spec,spec), s.albedo, m);
	s.fresnel = vec3(fresnel_intensity, fresnel_intensity, fresnel_intensity);
	s.roughness = 1.0 - g;
	s.roughness = max(s.roughness, MIN_ROUGHNESS);
	s.linearRoughness = s.roughness * s.roughness;
}
...
vec2 latlong(vec3 v) {
   float lantitude = acos(v.y) * 0.3183098916 + 0.5; // +y is up
   float longitude = atan(v.x, -v.z) * 0.1591549 + 0.5;
   return vec2(longitude, lantitude);
}
...
vec4 shader_main(){
	...
	state.vertex_tangent = gl_FrontFacing ? normalize(dPds_frag) : -(dPds_frag);
	...
	vec3 contrib0 = StandardSurfaceShading(state, main_light, 1.0);
	vec3 ibl_contribute = vec3(0.0,0.0,0.0);
	EvaluateIBL(state,ibl_contribute);
	vec3 finalColor = contrib0 + ibl_contribute;

	// add reflection
	state.reflect_dir = reflect(-state.vertex_eye, state.normal);
    float F = min(1.0, exp2( -9.28 * state.NoV)) + refl_f0;
	vec3 reflect_dir =  rotate(state.reflect_dir, refl_rotate);
    float longitude = atan(reflect_dir.x, reflect_dir.z) * 0.159155 + 0.5;
	float latitude  = atan(reflect_dir.y / sqrt(reflect_dir.x*reflect_dir.x + reflect_dir.z*reflect_dir.z)) * 0.31831 + 0.5;
	vec2 sample_refl_uv = vec2(longitude, 1.0-latitude);
    //sample_refl_uv = state.main_uv;
	vec3 refl = SAMPLER_2D(tex_refl, sample_refl_uv).rgb;
    //vec3 refl = SAMPLER_2D(tex_refl, state.main_uv).rgb;
	refl = sRGBToLinearFast(refl) * F * refl_intensity;
	finalColor = finalColor + refl;
