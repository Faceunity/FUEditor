...
vec3 StandardSurfaceShading(const FragmentState state, const Light light, const float visibility){
    ...
    float NoV = state.NoV;
    float NoL = saturate(dot(state.normal, light.l))+0.001;
    float NoH = saturate(dot(state.normal, h));
    float LoH = saturate(dot(light.l, h));
    ...
    return (color * light.colorIntensity.rgb) * (light.attenuation * NoL * visibility);
}
...
void EvaluateIBL(FragmentState state, inout vec3 color){
    ...
    vec3 r = getReflectedVector(state, n);
    ...
    //vec3 Fr = IBLReflection(state, state.NoV, r) * ao;

    // Note: iblLuminance is already premultiplied by the exposure
    //color.rgb += (Fd + Fr) * env_intensity;
    color.rgb += Fd * env_intensity;
}

void SetupMaterial(inout FragmentState state)
{
	vec4 base_color = SAMPLER_2D(tex_base_color, st_frag);
	state.albedo.rgb = sRGBToLinearFast(base_color.rgb);
	state.opacity = base_color.a;

	float m = 0.0;
	float g = 0.0;

	vec3 metallic_gloss_ao = SAMPLER_2D(tex_metallic_gloss_ao, state.main_uv).xyz;
	m = metallic_gloss_ao.r * metallic_factor;
    float g_linear = metallic_gloss_ao.g * gloss_factor;
    float g_srgb = sRGBToLinearFast(metallic_gloss_ao.g) * gloss_factor;
	g = (1.0-metallic_gloss_ao.g) * gloss_factor;
	state.ao = mix(1.0, metallic_gloss_ao.b, occlusion_intensity);
...
vec4 shader_main(){
	...
	//vec3 finalColor = ibl_contribute;
