...
- 区域8  
   ...
   ![](F:\FUEditor\TestBed\FUEditor\data\doc\img\quyu8.png)

   增加渲染选项：	

   1. ONCE：无论几个滤镜道具，只执行第一个，在道具渲染后执行
   2. PRE：在渲染道具之前执行Filter渲染
   3. NORMAL：执行所有滤镜道具渲染，在道具渲染后执行
   4. FXAA：使用抗锯齿功能
   5. 渲染选项打包后不可修改
...
3. 选中子特效时可以滑动g_param中的filterRatio来调整子特效的程度，这个程度就是和音乐强度绑定的值。
     ...
     g_scale为调试时调整当前图片大小的作用（在新添加的功能中无法使用，如3D眼镜等）
